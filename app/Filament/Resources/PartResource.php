        ...
        return $form
            ...
            ->schema([
                ...
                Section::make('Informacje o części')
                    ...
                    ->schema([
                        Forms\Components\TextInput::make('name')->label('Tytuł produktu')->placeholder('Tytuł produktu')->required()->maxLength(255)->live(onBlur: true)->afterStateUpdated(fn (Forms\Get $get, Forms\Set $set, ?Part $record): null => self::refreshCategorySuggestion($get, $set, $record))->columnSpanFull(),
                        Forms\Components\TextInput::make('sku')->label('Główny kod części')->placeholder('Główny kod części')->unique(ignoreRecord: true)->maxLength(255)->live(onBlur: true)->afterStateUpdated(fn (Forms\Get $get, Forms\Set $set, ?Part $record): null => self::refreshCategorySuggestion($get, $set, $record))->columnSpanFull(),
                        ...
                        Forms\Components\Select::make('condition_notes')->label('Jakość')->placeholder('Jakość')->options(['Używany' => 'Używany', 'Nowy' => 'Nowy', 'Uszkodzony' => 'Uszkodzony', 'Regenerowany' => 'Regenerowany'])->default('Używany')->native(false)->columnSpan(6),
                        ...
                        Forms\Components\Select::make('steering_side')->label('Kierownica po stronie')->placeholder('Kierownica po stronie')->options(['Lewej' => 'Lewej', 'Prawej' => 'Prawej'])->default('Lewej')->native(false)->dehydrated(false)->columnSpan(6),
