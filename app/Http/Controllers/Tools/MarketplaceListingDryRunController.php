...
class MarketplaceListingDryRunController extends Controller
{
    ...
    private function payloadFor(Part $part, string $channel, array $readiness): array
    {
        ...
        return array_merge($common, ['name' => $part->name, 'stock' => ['available' => (int) $part->quantity, 'quantity' => (int) $part->quantity], 'description' => $allegroDescription['description'], 'allegro_description_diagnostics' => $allegroDescription['diagnostics'], 'parameters' => $allegro['offer_parameters'] ?? [], 'productSet' => [['product' => ['parameters' => $allegro['product_parameters'] ?? []]]], 'allegro_parameter_diagnostics' => ['productSet[0].product.parameters' => $allegro['product_parameter_diagnostics'] ?? [], 'parameters' => $allegro['offer_parameter_diagnostics'] ?? [], 'all' => $allegro['parameter_source_diagnostics'] ?? []], 'allegro_parameters' => $allegro, 'allegro_product_parameters' => $allegro['product_parameters'] ?? [], 'allegro_offer_parameters' => $allegro['offer_parameters'] ?? [], 'missing_required_parameters' => $allegro['missing_required_parameters'] ?? [], 'unmapped_parameters' => $allegro['unmapped_parameters'] ?? [], 'parameter_definitions_source' => $allegro['parameter_definitions_source'] ?? 'none', 'will_make_marketplace_request' => false, 'shipping' => $this->accountSetting($channel, 'shipping'), 'payment' => $this->accountSetting($channel, 'payment'), 'return' => $this->accountSetting($channel, 'return'), 'delivery' => ['shippingRates' => ['id' => data_get($readiness, 'allegro_sales_settings.shippingRates.id')]], 'afterSalesServices' => array_filter(['returnPolicy' => ['id' => data_get($readiness, 'allegro_sales_settings.returnPolicy.id')], 'impliedWarranty' => ['id' => data_get($readiness, 'allegro_sales_settings.impliedWarranty.id')], 'warranty' => ['id' => data_get($readiness, 'allegro_sales_settings.warranty.id')]], fn ($row) => filled($row['id'] ?? null)), 'allegro_sales_settings' => $readiness['allegro_sales_settings'] ?? null]);
    }
