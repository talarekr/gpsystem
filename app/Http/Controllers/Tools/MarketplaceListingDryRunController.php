...
class MarketplaceListingDryRunController extends Controller
{
    ...
    private function imageUrls(Part $part): array { $selection = app(MarketplaceImageSelectionService::class)->selectForPart($part, 0, false, (string) request()->query('channel')); return ['count' => $part->images->count(), 'public_urls_sample' => $selection['urls'], 'missing_public_images_count' => max(0, $part->images->count() - count($selection['urls'])), 'diagnostics' => $selection['diagnostics']]; }
