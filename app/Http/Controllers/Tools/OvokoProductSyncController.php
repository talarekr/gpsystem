...
class OvokoProductSyncController extends Controller
{
    ...
    public function dryRun(Request $request): JsonResponse
    {
        ...
        foreach ($parts as $part) {
            ...
            if ($analysis['blockers'] !== []) {
                ...
                foreach ($analysis['blockers'] as $blocker) $summary['blockers'][$blocker] = ($summary['blockers'][$blocker] ?? 0) + 1;
    ...
    public function readiness(Request $request): JsonResponse
    {
        ...
        $sampleLimit = max(1, min(100, (int) $request->query('sample_limit', 20)));
        $result = ['ok' => true, 'ready_for_ovoko_create_count' => 0, 'already_has_ovoko_listing_count' => 0, 'blocked_count' => 0, 'top_blockers' => [], 'sample_ready' => [], 'sample_blocked' => [], 'warnings' => ['dry_run_only_no_ovoko_or_other_marketplace_writes']];
        ...
        $this->partsQuery($request)->chunkById(200, function ($parts) use (&$result, $sampleLimit): void {
            foreach ($parts as $part) {
                ...
                if ($analysis['blockers'] !== []) {
                    ...
                    foreach ($analysis['blockers'] as $blocker) $result['top_blockers'][$blocker] = ($result['top_blockers'][$blocker] ?? 0) + 1;
                } elseif (! $analysis['already_has_ovoko_listing']) {
    ...
    private function analysePart(Part $part, bool $includeAlreadyListed): array
    {
        ...
        $sample = ['part_id' => $part->id, 'part_number' => $part->part_number, 'name' => $part->name, 'blockers' => array_values(array_unique($blockers)), 'warnings' => array_values(array_unique($warnings))];
    ...
    private function emptyDryRunSummary(string $mode, int $page, int $limit): array { return ['ok' => true, 'dry_run' => true, 'local_update_only' => false, 'ovoko_write' => false, 'mode' => $mode, 'page' => $page, 'limit' => $limit, 'local_candidate_parts_count' => 0, 'already_has_ovoko_listing_count' => 0, 'would_create_ovoko_count' => 0, 'blocked_count' => 0, 'warning_count' => 0, 'sample_would_create' => [], 'sample_already_listed' => [], 'sample_blocked' => [], 'sample_payloads' => [], 'required_fields' => self::REQUIRED_FIELDS, 'blockers' => [], 'warnings' => ['dry_run_only_no_ovoko_or_other_marketplace_writes' => 1]]; }
}
