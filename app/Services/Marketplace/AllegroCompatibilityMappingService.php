...
class AllegroCompatibilityMappingService
{
    ...
    {
        [$rows, $responseKey] = $this->supportedCategoryRows($payload);
        $rows = array_values(array_filter($rows, 'is_array'));
        $supported = [];
        $normalized = [];
        foreach ($rows as $row) {
            $rowId = $this->supportedCategoryId($row);
            if ($rowId === '') continue;
            $supported[$rowId] = $row;
            $normalized[] = $this->supportedCategoryDiagnosticRow($row, $rowId);
        }
        $path = $this->allegroCategoryPath($id);
        $ids = array_map('strval', array_column($path, 'id'));
        $exact = isset($supported[(string) $id]);
        $matchedId = $exact ? (string) $id : null;
        foreach ($ids as $ancestorId) if (! $exact && $ancestorId !== (string) $id && isset($supported[$ancestorId])) { $matchedId = $ancestorId; break; }
        $row = $matchedId ? $supported[$matchedId] : [];
        $contains620 = isset($supported['620']);
        return ['category_supports_compatibility' => (bool) $matchedId, 'current_category_path' => implode(' > ', array_filter(array_column($path, 'name'))) ?: null, 'current_category_ancestors' => array_values(array_filter($path, fn ($x) => $x['id'] !== $id)), 'supported_category_exact_match' => $exact, 'supported_category_parent_match' => ! $exact && (bool) $matchedId, 'matched_supported_category_id' => $matchedId, 'matched_supported_category_name' => data_get($row, 'name', data_get($row, 'category.name')), 'matched_supported_category_input_type' => data_get($row, 'inputType', data_get($row, 'input.type')), 'matched_supported_category_items_type' => data_get($row, 'itemsType', data_get($row, 'items.type')), 'supported_categories_count' => count($normalized), 'supported_categories_sample' => array_slice($normalized, 0, 20), 'supported_categories_car_matches' => array_values(array_filter($normalized, fn ($x) => $this->isCarSupportedCategoryDiagnosticRow($x))), 'supported_categories_contains_category_id_620' => $contains620, 'supported_categories_category_id_620' => $contains620 ? $this->supportedCategoryDiagnosticRow($supported['620'], '620') : null, 'supported_categories_response_key' => $responseKey, 'supported_categories_diagnostic_note' => $matchedId ? null : ($contains620 ? 'category_620_exists_but_was_not_in_local_ancestor_path' : 'category_620_not_returned_by_allegro_supported_categories; inspect supported_categories_car_matches to see returned CAR categories')];
    }
    private function supportedCategoryRows(array $payload): array
    {
        foreach (['categories', 'supportedCategories', 'supported_categories'] as $key) {
            if (isset($payload[$key]) && is_array($payload[$key])) return [$payload[$key], $key];
        }
        return [array_is_list($payload) ? $payload : [], array_is_list($payload) ? 'root_list' : null];
    }
    private function supportedCategoryId(array $row): string
    {
        $id = data_get($row, 'id', data_get($row, 'category.id', data_get($row, 'categoryId')));
        return filled($id) ? (string) $id : '';
    }
    private function supportedCategoryDiagnosticRow(array $row, ?string $id = null): array
    {
        return ['id' => $id ?? $this->supportedCategoryId($row), 'raw_id' => data_get($row, 'id', data_get($row, 'category.id', data_get($row, 'categoryId'))), 'raw_id_type' => gettype(data_get($row, 'id', data_get($row, 'category.id', data_get($row, 'categoryId')))), 'name' => data_get($row, 'name', data_get($row, 'category.name')), 'inputType' => data_get($row, 'inputType', data_get($row, 'input.type')), 'itemsType' => data_get($row, 'itemsType', data_get($row, 'items.type'))];
    }
    private function isCarSupportedCategoryDiagnosticRow(array $row): bool
    {
        $name = mb_strtolower((string) ($row['name'] ?? ''));
        return strtoupper((string) ($row['itemsType'] ?? '')) === 'CAR' || str_contains($name, 'części samochodowe') || str_contains($name, 'samochod');
    }
