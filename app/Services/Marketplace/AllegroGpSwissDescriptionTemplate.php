...
class AllegroGpSwissDescriptionTemplate
{
    ...
    public function render(string $partDescription, array $vehicleValues, string $mainImageUrl): array
    {
        $content = '<p>Witam oferta dotyczy:</p>'
            . $this->partDescriptionHtml($partDescription)
            .'<ul>'
    ...
    private function partDescriptionHtml(string $partDescription): string
    {
        $paragraphs = preg_split('/\R{2,}/u', trim($partDescription)) ?: [];
        $html = '';

        foreach ($paragraphs as $paragraph) {
            $lines = array_values(array_filter(array_map('trim', preg_split('/\R/u', $paragraph) ?: []), fn (string $line): bool => $line !== ''));

            if ($lines === []) {
                continue;
            }

            $html .= '<p><b>'.implode('<br />', array_map(fn (string $line): string => $this->e($line), $lines)).'</b></p>';
        }
