...
class BuildAllegroMappingsFromPartsService
{
    ...
    private function writeMappedListing(string $offerId, array $row): void
    {
        ...
        $listing = MarketplaceListing::query()->updateOrCreate(['marketplace'=>'allegro','external_offer_id'=>$offerId], [
            'marketplace_account_id'=>$account->id,'part_id'=>$row['id'],'external_listing_id'=>$offerId,'sku'=>$row['sku'],'title'=>$row['name'],'price'=>is_numeric($row['price'])?(float)$row['price']:null,'quantity'=>is_numeric($row['quantity'])?(int)$row['quantity']:null,'currency'=>$offer['currency'] ?? 'PLN','status'=>$offer['status'] ?? 'imported','url'=>$offer['canonical_url'] ?? $offer['url'],'sync_status'=>'mapped','match_status'=>'confirmed','match_confidence'=>100,'match_reason'=>'primary allegro offer id from parts.legacy_payload_json','raw_payload'=>['source'=>'parts.legacy_payload.legacy_payload_json','allegro_offer_id'=>$offerId,'canonical_url'=>$offer['canonical_url'] ?? $offer['url'],'part_id'=>$row['id'],'offer'=>$offer],'last_error'=>null,'last_synced_at'=>now(),
        ]);
        MarketplaceSyncLog::query()->create(['marketplace'=>'allegro','marketplace_listing_id'=>$listing->id,'part_id'=>$listing->part_id,'action'=>'build_allegro_mapping_from_parts','status'=>'success','message'=>'primary allegro offer id from parts.legacy_payload_json','payload'=>['external_offer_id'=>$offerId,'part_id'=>$listing->part_id],'created_at'=>now()]);
    }

    /** @param array<int, array<string, mixed>> $rows */
    private function writeConflictListing(string $offerId, array $rows): void
    {
        $listing = MarketplaceListing::query()->updateOrCreate(['marketplace'=>'allegro','external_offer_id'=>$offerId], ['part_id'=>null,'sku'=>null,'title'=>'Conflict: duplicate Allegro offer ID '.$offerId,'price'=>null,'quantity'=>null,'currency'=>$rows[0]['offer']['currency'] ?? 'PLN','status'=>'imported','url'=>$rows[0]['offer']['canonical_url'] ?? $rows[0]['offer']['url'] ?? null,'sync_status'=>'conflict','match_status'=>'conflict','match_confidence'=>0,'match_reason'=>'duplicate primary allegro offer id in parts.legacy_payload_json','raw_payload'=>['source'=>'parts.legacy_payload.legacy_payload_json','allegro_offer_id'=>$offerId,'canonical_url'=>$rows[0]['offer']['canonical_url'] ?? $rows[0]['offer']['url'] ?? null,'part_ids'=>array_column($rows, 'id'),'count'=>count($rows),'offers'=>array_column($rows, 'offer')],'last_error'=>'Duplicate primary Allegro offer ID found in multiple parts; not mapped automatically.','last_synced_at'=>now()]);
        MarketplaceSyncLog::query()->create(['marketplace'=>'allegro','marketplace_listing_id'=>$listing->id,'part_id'=>null,'action'=>'build_allegro_mapping_from_parts','status'=>'conflict','message'=>'duplicate primary allegro offer id in parts.legacy_payload_json','payload'=>['external_offer_id'=>$offerId,'part_ids'=>array_column($rows, 'id')],'created_at'=>now()]);
    }
