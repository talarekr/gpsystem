...
class EbayLegacyCategoryMappingImportService
{
    ...
    public function check(): array
    {
        ...
        return ['ok'=>true,'total'=>DB::table('marketplace_category_mappings')->count(),'per_channel_count'=>DB::table('marketplace_category_mappings')->select('channel',DB::raw('count(*) as c'))->groupBy('channel')->pluck('c','channel'),'ebay_total'=>$this->channelCount('ebay'),'ebay_de_total'=>$this->channelCount('ebay_de'),'ebay_fr_total'=>$this->channelCount('ebay_fr'),'used_local_categories_total'=>count($used),'used_local_categories_with_mapping'=>$mappedUsed,'used_local_categories_missing_mapping'=>count($missing),'blocked_count'=>DB::table('marketplace_category_mappings')->where('is_blocked', true)->count(),'missing_external_category_id_count'=>DB::table('marketplace_category_mappings')->whereIn('channel', self::CHANNELS)->whereNull('external_category_id')->count(),'sample_mappings'=>DB::table('marketplace_category_mappings')->whereIn('channel', self::CHANNELS)->orderBy('id')->limit(20)->get(),'sample_missing_used_categories'=>$this->missingSamples($missing),'warnings'=>['Read-only check; no eBay API calls are performed.'],'blockers'=>[]];
    }
