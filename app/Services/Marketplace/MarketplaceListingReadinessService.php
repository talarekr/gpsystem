...
class MarketplaceListingReadinessService
{
    ...
    public function checkPartReadiness(Part $part, string $channel): array
    {
        ...
        if ($channel !== 'storefront' && $this->marketplaceImageSelectionService->selectForPart($part, 0, false, $channel)['urls'] === []) {
            $missing[] = 'images';
        ...
        if ($channel === 'storefront') {
            ...
            $this->checkEbayAccount($account, $channel, $blockers, $warnings);
    ...
    public function prepareEbayTranslations(Part $part, string $channel): array
    {
        ...
        if ($channel === 'ebay_de') {
            ...
            // Keep the Google-translated title 1:1 in prepared fields; sanitizer only reports diagnostics and a suggested short title.
    ...
    private function checkEbayAccount(?MarketplaceAccount $account, string $channel, array &$blockers, array &$warnings): void { if (! $account || ! ($account->api_enabled ?? false)) $blockers[] = 'eBay API/settings are not configured or not enabled.'; else $this->checkAccount($account, $blockers, $warnings, 'eBay API/settings are not configured or not enabled.'); $settings = is_array($account?->api_settings) ? $account->api_settings : []; $expected = $channel === 'ebay_fr' ? 'EBAY_FR' : 'EBAY_DE'; $configured = filled($settings['marketplace_id'] ?? null) ? (string) $settings['marketplace_id'] : null; if ($configured !== $expected) $blockers[] = 'eBay marketplace_id for '.$channel.' must be '.$expected.'.'; }
    ...
    private function safePayloadPreview(Part $part, string $channel, ?float $price, ?MarketplaceCategoryMapping $categoryMapping = null, ?array $ebayPrice = null, ?array $allegroParameters = null, ?array $allegroSalesSettings = null): array
    {
        ...
        if ($channel === 'allegro_main' && $allegroParameters !== null) {
            ...
            $preview['afterSalesServices'] = array_filter([
                ...
                'warranty' => ['id' => $allegroSalesSettings['warranty']['id'] ?? null],
            ], fn ($row) => filled($row['id'] ?? null));
        }
