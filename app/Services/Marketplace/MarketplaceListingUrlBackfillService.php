...
class MarketplaceListingUrlBackfillService
{
    ...
    public function run(string $marketplace, ?string $channel = null, bool $apply = false, int $limit = 100, int $offset = 0, bool $onlyMissing = false, bool $includeExistingInvalid = false, ?int $listingId = null, ?int $partId = null): array
    {
        ...
        $summary = [
            ...
            'invalid_external_id' => 0, 'missing_external_id' => 0, 'suspicious_existing_url' => 0, 'image_url_rejected' => 0, 'skipped' => 0,
        ];
        $results = [];
        $ids = [];
        ...
        foreach ($rows as $listing) {
            ...
            $generatedUrl = $resolvedId !== null && preg_match('/^\d+$/', $resolvedId) === 1 ? $this->generateUrl($marketplace, $listing, $resolvedId) : null;
            ...
            if ($existingUrl !== null && $validExisting) {
                ...
                } elseif (preg_match('/^\d+$/', $resolvedId) !== 1 || $generatedUrl === null) {
            ...
            $results[] = [
                ...
                'resolved_marketplace_item_id' => $resolvedId, 'existing_url' => $existingUrl, 'generated_url' => $generatedUrl, 'action' => $action, 'reason' => $reason,
            ];
        }
