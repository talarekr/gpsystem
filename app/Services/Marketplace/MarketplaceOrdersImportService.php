...
class MarketplaceOrdersImportService
{
    ...
    private function localStatus(string $status): string { $normalized = strtoupper(trim($status)); return match ($normalized) { 'CANCELLED', 'CANCELED' => 'cancelled', 'SUSPENDED' => 'on_hold', default => str_contains(strtolower($status), 'complete') ? 'completed' : 'new', }; }
