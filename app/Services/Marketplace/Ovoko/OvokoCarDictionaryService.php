...
class OvokoCarDictionaryService
{
    ...
    private function storeRows(string $dictionary, array $rows, ?string $brandId = null): int { $count = 0; $brandKey = (string) ($brandId ?? ''); foreach ($rows as $row) { $id = (string) ($row['id'] ?? $row['value'] ?? $row['code'] ?? $row['car_model_id'] ?? $row['model_id'] ?? ''); if ($id === '') continue; OvokoCarDictionaryEntry::query()->updateOrCreate(['dictionary' => $dictionary, 'ovoko_id' => $id, 'ovoko_brand_id' => $brandKey], ['name' => $row['name'] ?? $row['title'] ?? $row['label'] ?? $row['value_name'] ?? null, 'year_from' => $dictionary === 'models' ? $this->normalizeNullableYear($row['year_from'] ?? $row['from_year'] ?? $row['year_start'] ?? null) : ($row['year_from'] ?? $row['from_year'] ?? $row['year_start'] ?? null), 'year_to' => $dictionary === 'models' ? $this->normalizeNullableYear($row['year_to'] ?? $row['to_year'] ?? $row['year_end'] ?? null) : ($row['year_to'] ?? $row['to_year'] ?? (filled($row['year_end'] ?? null) ? $row['year_end'] : null)), 'raw_payload' => $row, 'synced_at' => now()]); $count++; } return $count; }
