...
class OvokoCarDictionaryService
{
    ...
    public function storeRows(string $dictionary, array $rows, ?string $brandId = null): int { $count = 0; $brandKey = $this->normalizeDictionaryId($brandId); foreach ($rows as $row) { $id = $this->normalizeDictionaryId($row['id'] ?? $row['value'] ?? $row['code'] ?? $row['car_model_id'] ?? $row['model_id'] ?? null); if ($id === '') continue; OvokoCarDictionaryEntry::query()->updateOrCreate(['dictionary' => $dictionary, 'ovoko_id' => $id, 'ovoko_brand_id' => $brandKey], ['name' => $this->extractRowName($row, $id), 'year_from' => $dictionary === 'models' ? $this->normalizeNullableYear($row['year_from'] ?? $row['from_year'] ?? $row['year_start'] ?? null) : ($row['year_from'] ?? $row['from_year'] ?? $row['year_start'] ?? null), 'year_to' => $dictionary === 'models' ? $this->normalizeNullableYear($row['year_to'] ?? $row['to_year'] ?? $row['year_end'] ?? null) : ($row['year_to'] ?? $row['to_year'] ?? (filled($row['year_end'] ?? null) ? $row['year_end'] : null)), 'raw_payload' => $row, 'synced_at' => now()]); $count++; } return $count; }
    private function normalizeDictionaryId(mixed $value): string { if ($value === null) return ''; if (is_scalar($value) || $value instanceof \Stringable) return trim((string) $value); return ''; }
    private function extractRowName(array $row, string $id): string { foreach (['name', 'status_name', 'pl', 'en', 'label', 'title', 'value'] as $field) { $value = $row[$field] ?? null; if ((is_string($value) || is_numeric($value)) && trim((string) $value) !== '') return trim((string) $value); } return $id; }
    private function normalizeNullableYear(mixed $value): ?int { if (is_int($value)) return $value > 0 ? $value : null; if (is_float($value)) return $value > 0 && floor($value) === $value ? (int) $value : null; if (! is_string($value)) return null; $value = trim($value); if ($value === '' || ! ctype_digit($value)) return null; $year = (int) $value; return $year > 0 ? $year : null; }
