...
class OvokoPriceSyncAdapter implements MarketplacePriceSyncAdapter
{
    public function sync(MarketplaceListing $listing, array $price): array
    {
        ...
        return ['status'=>$ok?'success':'error','http_status'=>$write->status(),'api_business_status'=>$api,'endpoint'=>'POST /crm/updatePart','read_after_write_endpoint'=>'POST /get/part/{id}','request_summary'=>$safeReq,'response_summary'=>$this->safe($body),'read_after_write'=>$this->safe($raw)+['confirmed_pln_price'=>$confirmed],'remote_confirmed_price'=>$ok?$confirmed:null,'confirmed_remote_price'=>$ok?$confirmed:null,'final_success'=>$ok,'blocker'=>$ok?null:'ovoko_read_after_write_price_mismatch'];
    }
    public function partRow(array $raw): array
    {
        foreach (['part','item','data','result','list'] as $key) {
            if (array_key_exists($key, $raw)) {
                $row = $this->unwrap($raw[$key]);
                if ($this->looksLikePartRow($row)) return $row;
            }
        }
        $row = $this->unwrap($raw);
    ...
    {
        while (is_array($value) && array_is_list($value)) $value = $value[0] ?? [];
        return is_array($value) ? $value : [];
    }
    private function looksLikePartRow(array $row): bool { return isset($row['original_price']) || isset($row['price']) || isset($row['id']) || isset($row['part_id']); }
    public function confirmedPlnPrice(array $row): ?string { $n=app(PriceNormalizer::class); if (strtoupper((string)($row['original_currency']??''))==='PLN' && isset($row['original_price'])) return $n->normalize($row['original_price']); if (strtoupper((string)($row['currency']??''))==='PLN' && isset($row['price'])) return $n->normalize($row['price']); return null; }
