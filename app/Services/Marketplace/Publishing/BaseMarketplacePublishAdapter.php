...
abstract class BaseMarketplacePublishAdapter implements MarketplacePublishAdapterInterface
{
    ...
    public function publish(Part $part, MarketplacePublishCommand $command): MarketplacePublishResult
    {
        ...
        if (! ($result['ok'] ?? false)) {
            ...
            return new MarketplacePublishResult($this->channel(), ['channel' => $this->channel(), 'marketplace' => $this->marketplace(), 'success' => false, 'status' => $result['status'] ?? 'api_error', 'errors' => [$result['error'] ?? 'marketplace_api_error'], 'warnings' => [], 'write' => false]);
        }

        $listing = MarketplaceListing::query()->create(['marketplace' => $this->marketplace(), 'marketplace_account_id' => $account?->id, 'part_id' => $part->id, 'external_offer_id' => $result['external_offer_id'] ?? $result['offer_id'] ?? null, 'external_listing_id' => $result['external_listing_id'] ?? $result['listing_id'] ?? null, 'external_inventory_id' => $result['external_inventory_id'] ?? ($payload['sku'] ?? $part->sku), 'sku' => $payload['sku'] ?? $part->sku, 'title' => $payload['title'] ?? $part->name, 'price' => $readiness['marketplace_price'] ?? $part->price, 'quantity' => $part->quantity, 'currency' => $readiness['currency'] ?? 'PLN', 'status' => $result['listing_status'] ?? 'published', 'sync_status' => 'published', 'match_status' => 'matched', 'match_confidence' => 100, 'url' => $result['url'] ?? null, 'raw_payload' => ['request_summary' => $this->requestSummary($payload), 'response_summary' => $result['response_summary'] ?? []], 'last_synced_at' => now()]);
        $this->logger->success($this->marketplace(), $result['action'] ?? 'publish', 'Marketplace publish API call completed.', ['marketplace_listing_id' => $listing->id, 'part_id' => $part->id, 'http_status' => $result['http_status'] ?? null, 'duration_ms' => $duration, 'external_id' => $listing->external_listing_id ?: $listing->external_offer_id, 'request_id' => $result['request_id'] ?? null, 'request' => $this->requestSummary($payload), 'response' => $result['response_summary'] ?? []]);
