...
abstract class BaseMarketplacePublishAdapter implements MarketplacePublishAdapterInterface
{
    ...
    private function publishWithoutLock(Part $part, MarketplacePublishCommand $command): MarketplacePublishResult
    {
        ...
        $listingData = ['marketplace_account_id' => $account?->id, 'external_offer_id' => $result['external_offer_id'] ?? $result['offer_id'] ?? $listing?->external_offer_id ?? null, 'external_listing_id' => $result['external_listing_id'] ?? $result['listing_id'] ?? $listing?->external_listing_id ?? null, 'external_inventory_id' => $result['external_inventory_id'] ?? $resolvedSku, 'sku' => $resolvedSku, 'title' => $payload['title'] ?? $part->name, 'price' => $readiness['marketplace_price'] ?? $part->price, 'quantity' => $part->quantity, 'currency' => $readiness['currency'] ?? 'PLN', 'status' => $result['listing_status'] ?? 'published', 'sync_status' => $result['sync_status'] ?? 'published', 'match_status' => $result['match_status'] ?? 'matched', 'match_confidence' => 100, 'url' => $result['url'] ?? $listing?->url ?? null, 'raw_payload' => ['request_summary' => $requestSummary, 'response_summary' => $result['response_summary'] ?? [], 'metadata' => $result['metadata'] ?? []], 'last_api_status' => $result['last_api_status'] ?? 'published', 'last_error' => null, 'last_synced_at' => now()];
        if (! $listing) {
            $listing = MarketplaceListing::query()->create(array_merge(['marketplace' => $this->marketplace(), 'part_id' => $part->id], $listingData));
        } else {
            $listing->forceFill($listingData)->save();
        }
