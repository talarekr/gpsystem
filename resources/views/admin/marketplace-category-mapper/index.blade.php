                ...
                <div class="list">
                    <template x-for="item in column.items" :key="column.code+'-'+item.id">
                        <button type="button" class="row" :class="{'is-selected': isSelected(column.code,item)}" @click="choose(column,item)">
                            <span><strong x-text="item.name"></strong><small x-text="item.products_count !== undefined && item.products_count !== null ? item.products_count+' produktów' : item.path"></small></span>
