...
<thead><tr><th>part_id</th><th>SKU</th><th>classification</th><th>duplicate_guard_would_block</th><th>eBay statusy</th><th>resolver</th></tr></thead>
<tbody>
@foreach(($rows ?? []) as $row)
<tr><td>{{ $row['part']['id'] }}</td><td>{{ $row['part']['sku'] }}</td><td>{{ $row['audit_classification'] }}</td><td>{{ $row['duplicate_guard_would_block'] ? 'yes' : 'no' }}</td><td>{{ collect($row['marketplace_listings'])->map(fn($l) => ($l['api']['api_listing_status'] ?? '').' endPast='.json_encode($l['api']['end_date_is_past'] ?? null))->implode('; ') }}</td><td>{{ $row['resolver_ebay']['display_icon'] ?? '' }} {{ $row['resolver_ebay']['reason'] ?? '' }}</td></tr>
@endforeach
</tbody>
</table>
...
function updateDownloads(){
 ...
 const csv=['part_id,sku,classification,duplicate_guard_would_block'].concat(allRows.map(r=>[r.part.id,r.part.sku,r.audit_classification,r.duplicate_guard_would_block].map(v=>'"'+String(v??'').replaceAll('"','""')+'"').join(','))).join('\n');
