...
function updateDownloads(){
 ...
 const csv=['part_id,sku,ebay_de_status,ebay_de_url,ebay_fr_status,ebay_fr_url,listing_exists,endPast,needs_ebay_de_publish,ebay_overall,classification,duplicate_guard_would_block'].concat(allRows.map(r=>[r.part.id,r.part.sku,r.ebay_de_status,r.ebay_de_url,r.ebay_fr_status,r.ebay_fr_url,channelListings(r).some(l=>l.listing_exists),channelListings(r).map(l=>l.api?.end_date_is_past??'').join('; '),r.needs_ebay_de_publish,r.ebay_overall,r.audit_classification,r.duplicate_guard_would_block].map(v=>'"'+String(v??'').replaceAll('"','""')+'"').join(','))).join('\n');
