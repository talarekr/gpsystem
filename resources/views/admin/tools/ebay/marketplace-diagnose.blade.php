...
<thead><tr><th>part_id</th><th>SKU</th><th>ebay_de_status</th><th>ebay_de_url</th><th>ebay_fr_status</th><th>ebay_fr_url</th><th>needs_ebay_de_publish</th><th>classification</th><th>duplicate_guard_would_block</th><th>eBay statusy</th><th>resolver</th></tr></thead>
<tbody>
@foreach(($rows ?? []) as $row)
<tr><td>{{ $row['part']['id'] }}</td><td>{{ $row['part']['sku'] }}</td><td>{{ $row['ebay_de_status'] ?? '' }}</td><td>{{ $row['ebay_de_url'] ?? '' }}</td><td>{{ $row['ebay_fr_status'] ?? '' }}</td><td>{{ $row['ebay_fr_url'] ?? '' }}</td><td>{{ ($row['needs_ebay_de_publish'] ?? false) ? 'true' : 'false' }}</td><td>{{ $row['audit_classification'] }}</td><td>{{ $row['duplicate_guard_would_block'] ? 'yes' : 'no' }}</td><td>{{ collect($row['marketplace_listings'])->map(fn($l) => ($l['api']['api_listing_status'] ?? '').' endPast='.json_encode($l['api']['end_date_is_past'] ?? null))->implode('; ') }}</td><td>{{ $row['resolver_ebay']['display_icon'] ?? '' }} {{ $row['resolver_ebay']['reason'] ?? '' }}</td></tr>
@endforeach
</tbody>
</table>
...
function updateDownloads(){
 ...
 const csv=['part_id,sku,ebay_de_status,ebay_de_url,ebay_fr_status,ebay_fr_url,needs_ebay_de_publish,ebay_overall,classification,duplicate_guard_would_block'].concat(allRows.map(r=>[r.part.id,r.part.sku,r.ebay_de_status,r.ebay_de_url,r.ebay_fr_status,r.ebay_fr_url,r.needs_ebay_de_publish,r.ebay_overall,r.audit_classification,r.duplicate_guard_would_block].map(v=>'"'+String(v??'').replaceAll('"','""')+'"').join(','))).join('\n');
...
function addRows(rows){const tbody=document.querySelector('#resultsTable tbody'); rows.forEach(r=>{allRows.push(r); if(r.audit_classification==='active OK')counters.active_OK++; if(r.audit_classification.includes('ended/stale'))counters.ended_stale++; if(r.audit_classification.includes('not_found'))counters.not_found++; if(r.audit_classification.includes('api_error'))counters.api_error++; if(r.audit_classification.includes('needs_review'))counters.needs_review++; if(r.duplicate_guard_would_block)counters.duplicate_guard_would_block++; const tr=document.createElement('tr'); tr.innerHTML=`<td>${r.part.id}</td><td>${r.part.sku??''}</td><td>${r.ebay_de_status??''}</td><td>${r.ebay_de_url??''}</td><td>${r.ebay_fr_status??''}</td><td>${r.ebay_fr_url??''}</td><td>${r.needs_ebay_de_publish?'true':'false'}</td><td>${r.audit_classification}</td><td>${r.duplicate_guard_would_block?'yes':'no'}</td><td>${r.marketplace_listings.map(l=>(l.api.api_listing_status||'')+' endPast='+l.api.end_date_is_past).join('; ')}</td><td>${r.resolver_ebay.display_icon??''} ${r.resolver_ebay.reason??''}</td>`; tbody.appendChild(tr);}); updateDownloads();}
