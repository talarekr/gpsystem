...
<thead><tr><th>part_id</th><th>SKU</th><th>ebay_de_status</th><th>ebay_de_url</th><th>ebay_fr_status</th><th>ebay_fr_url</th><th>listing_exists</th><th>endPast</th><th>public_item_id</th><th>seller_offer_id</th><th>seller_listing_id</th><th>requested_listing_id</th><th>offer_listing_id</th><th>seller_listing_id_matches_public_item_id</th><th>public_item_end_date</th><th>public_item_end_date_source</th><th>public_item_end_past</th><th>raw payload/date diagnostics</th><th>seller_listing_status</th><th>seller_offer_status</th><th>needs_ebay_de_publish</th><th>classification</th><th>duplicate_guard_would_block</th><th>eBay statusy</th><th>resolver</th></tr></thead>
<tbody>
@foreach(($rows ?? []) as $row)
<tr><td>{{ $row['part']['id'] }}</td><td>{{ $row['part']['sku'] }}</td><td>{{ $row['ebay_de_status'] ?? '' }}</td><td>{{ $row['ebay_de_url'] ?? '' }}</td><td>{{ $row['ebay_fr_status'] ?? '' }}</td><td>{{ $row['ebay_fr_url'] ?? '' }}</td><td>{{ collect($row['marketplace_listings'])->whereIn('marketplace', ['ebay_de', 'ebay'])->contains('listing_exists', true) ? 'yes' : 'no' }}</td><td>{{ collect($row['marketplace_listings'])->whereIn('marketplace', ['ebay_de', 'ebay'])->map(fn($l) => json_encode($l['api']['end_date_is_past'] ?? null))->implode('; ') }}</td><td>{{ $row['public_item_id'] ?? '' }}</td><td>{{ $row['seller_offer_id'] ?? '' }}</td><td>{{ $row['seller_listing_id'] ?? '' }}</td><td>{{ $row['requested_listing_id'] ?? '' }}</td><td>{{ $row['offer_listing_id'] ?? '' }}</td><td>{{ json_encode($row['seller_listing_id_matches_public_item_id'] ?? null) }}</td><td>{{ $row['public_item_end_date'] ?? '' }}</td><td>{{ $row['public_item_end_date_source'] ?? '' }}</td><td>{{ json_encode($row['public_item_end_past'] ?? null) }}</td><td><pre>{{ json_encode(collect($row['marketplace_listings'])->whereIn('marketplace', ['ebay_de', 'ebay'])->map(fn($l) => ['id' => $l['id'], 'external_offer_id' => $l['external_offer_id'] ?? null, 'external_listing_id' => $l['external_listing_id'] ?? null, 'url' => $l['url'] ?? null, 'status' => $l['status'] ?? null, 'sync_status' => $l['sync_status'] ?? null, 'last_api_status' => $l['last_api_status'] ?? null, 'raw_payload_diagnostics' => $l['raw_payload_diagnostics'] ?? null])->values(), JSON_UNESCAPED_SLASHES|JSON_UNESCAPED_UNICODE) }}</pre></td><td>{{ $row['seller_listing_status'] ?? '' }}</td><td>{{ $row['seller_offer_status'] ?? '' }}</td><td>{{ array_key_exists('needs_ebay_de_publish', $row) && $row['needs_ebay_de_publish'] === null ? 'unknown' : (($row['needs_ebay_de_publish'] ?? false) ? 'true' : 'false') }}</td><td>{{ $row['audit_classification'] }}</td><td>{{ $row['duplicate_guard_would_block'] ? 'yes' : 'no' }}</td><td>{{ collect($row['marketplace_listings'])->map(fn($l) => ($l['api']['api_listing_status'] ?? '').' endPast='.json_encode($l['api']['end_date_is_past'] ?? null))->implode('; ') }}</td><td>{{ $row['resolver_ebay']['display_icon'] ?? '' }} {{ $row['resolver_ebay']['reason'] ?? '' }}</td></tr>
@endforeach
</tbody>
</table>
