        <form wire:submit="createDhlShipment" class="gps-dhl-form">
            ...
            <div class="gps-actions gps-action-bar"><button type="submit" class="gps-action gps-primary" wire:confirm="To utworzy live przesyłkę DHL createShipment. Kontynuować?">Utwórz przesyłkę DHL</button>@if($this instanceof \App\Filament\Pages\CreateShipment)<a href="{{ \App\Filament\Pages\Shipments::getUrl() }}" class="gps-action">Anuluj</a>@else<button type="button" class="gps-action" wire:click="$set('showDhlForm', false)">Anuluj</button>@endif</div>
        </form>