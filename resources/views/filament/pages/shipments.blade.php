...
<x-filament-panels::page>
    ...
    @if($showDhlForm)
        <form wire:submit="createDhlShipment" class="gps-dhl-form">
            ...
            <div class="gps-form-section"><h3>Dane odbiorcy</h3><div class="gps-form-grid">
                @foreach (['name'=>'Imię/nazwa','company'=>'Firma','country'=>'Kraj','postal_code'=>'Kod pocztowy','city'=>'Miejscowość','street'=>'Ulica','house_number'=>'Numer domu','apartment_number'=>'Numer lokalu','person_name'=>'Osoba kontaktowa','email'=>'Email','phone'=>'Telefon'] as $key => $label)
                    <div class="gps-field"><label>{{ $label }}</label><input wire:model="dhlForm.receiver.{{ $key }}"></div>
                @endforeach
            </div></div>
