    ...
    <div class="space-y-3">
        ...
        @forelse ($cars as $car)
            <div class="gps-card gps-grid" wire:key="car-{{ $car->id }}">
                <div class="gps-cell"><div class="gps-car-main">#{{ $car->id }} {{ trim(implode(' ', array_filter([$car->make, $car->model]))) ?: '—' }}</div><div class="gps-muted">VIN: {{ $car->vin ?: '—' }} · Rej.: {{ $car->registration_number ?: '—' }}</div></div>
                <div class="gps-cell"><span class="gps-badge">{{ $car->status ?: '—' }}</span></div><div class="gps-cell gps-car-text">{{ $car->fuel_type ?: '—' }}</div><div class="gps-cell gps-car-text">{{ $car->gearbox_type ?: '—' }}</div><div class="gps-cell gps-car-text">{{ $car->engine_code ?: '—' }}@if($car->engine_capacity_cm3) <span class="gps-muted">{{ $car->engine_capacity_cm3 }} cm3</span>@endif</div><div class="gps-cell gps-car-text">{{ $car->purchase_date?->format('Y-m-d') ?? '—' }}</div><div class="gps-cell gps-car-text">{{ $car->createdBy?->name ?? '—' }}</div>
