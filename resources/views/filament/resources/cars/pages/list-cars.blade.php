    ...
    <div class="space-y-3">
        ...
        @forelse ($cars as $car)
            <div class="gps-card gps-grid" wire:key="car-{{ $car->id }}">
                ...
                <div class="gps-cell gps-car-text">{{ $car->fuel_type ?: '—' }}</div><div class="gps-cell gps-car-text">{{ $car->gearbox_type ?: '—' }}</div><div class="gps-cell gps-car-text">{{ $car->engine_code ?: '—' }}@if($car->engine_capacity_cm3) <span class="gps-muted">{{ $car->engine_capacity_cm3 }} cm3</span>@endif</div><div class="gps-cell gps-car-text">{{ $car->production_year ?: '—' }}</div><div class="gps-cell gps-car-text">@if($car->engine_power_kw){{ $car->engine_power_kw }} kW@else—@endif</div><div class="gps-cell gps-car-text">{{ $car->drivetrain ?: '—' }}</div><div class="gps-cell gps-car-text">{{ $car->body_type ?: '—' }}</div><div class="gps-cell gps-car-text">{{ $car->color ?: '—' }}</div>
