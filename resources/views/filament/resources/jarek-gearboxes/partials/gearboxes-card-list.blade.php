...
<div class="gps-parts-list">
    <div class="gps-parts-list-header gps-admin-parts-grid"><div>Skrzynia</div><div>Numer / identyfikator</div><div>Kanały sprzedaży</div><div>Mapowanie</div><div>Status</div><div>Import</div><div>ID</div><div>Akcje</div></div>
    @forelse ($gearboxes as $gearbox)
        ...
        <div class="gps-part-card"><div class="gps-admin-parts-grid">
            ...
            <div class="gps-part-col"><div class="gps-part-number-row"><span class="gps-part-number">{{ $identifier ?: '—' }}</span>@if (filled($identifier))<button type="button" class="gps-part-copy" title="Kopiuj identyfikator" onclick="event.preventDefault(); navigator.clipboard?.writeText(@js((string) $identifier));"><svg viewBox="0 0 20 20" fill="currentColor"><path d="M7 3.5A2.5 2.5 0 0 1 9.5 1h6A2.5 2.5 0 0 1 18 3.5v6a2.5 2.5 0 0 1-2.5 2.5h-6A2.5 2.5 0 0 1 7 9.5v-6Z"/><path d="M4.5 6A2.5 2.5 0 0 0 2 8.5v8A2.5 2.5 0 0 0 4.5 19h8a2.5 2.5 0 0 0 2.5-2.5V14h-2v2.5a.5.5 0 0 1-.5.5h-8a.5.5 0 0 1-.5-.5v-8a.5.5 0 0 1 .5-.5H7V6H4.5Z"/></svg></button>@endif</div></div>
            ...
            <div class="gps-part-col"><div class="gps-part-muted">Import: {{ $gearbox->imported_at?->format('Y-m-d H:i') ?: '—' }}</div><div class="gps-part-muted">Akt. Allegro: {{ $gearbox->updated_from_allegro_at?->format('Y-m-d H:i') ?: '—' }}</div></div>
            <div class="gps-part-col"><div class="gps-part-id">{{ $gearbox->id }}</div></div>
            <div class="gps-part-col"><div class="gps-part-actions"><a class="gps-part-action" href="{{ JarekGearboxResource::getUrl('edit', ['record' => $gearbox]) }}">Edytuj</a><a class="gps-part-action" href="{{ JarekGearboxResource::getUrl('view', ['record' => $gearbox]) }}">Podgląd</a><a class="gps-part-action" href="{{ $ebayPreviewUrl }}" target="_blank" rel="noopener">eBay preview</a></div></div>
        </div></div>
    @empty
