@php
    ...
    $parts = $this->parts;
    $listContextBadge = $this->getListContextBadge();
@endphp

<x-filament-panels::page>
    <style>
        .gps-parts-toolbar{display:grid;grid-template-columns:minmax(280px,1fr) minmax(180px,auto) minmax(110px,auto) minmax(110px,auto) auto;gap:12px;align-items:end;margin-bottom:16px}.gps-parts-field{display:flex;flex-direction:column;gap:6px;min-width:0}.gps-parts-field label{font-size:12px;font-weight:700;color:#64748b}.gps-parts-field input,.gps-parts-field select{width:100%;border:1px solid #d1d5db;border-radius:10px;background:#fff;padding:9px 12px;font-size:14px;color:#0f172a}.gps-parts-filter-summary{display:inline-flex;align-items:center;justify-content:center;min-height:40px;border-radius:10px;background:#eff6ff;color:#1d4ed8;font-weight:800;padding:0 12px;white-space:nowrap}.gps-parts-button,.gps-parts-reset-button{min-height:40px;border-radius:10px;border:1px solid #d1d5db;padding:0 14px;font-weight:800;color:#334155;background:#fff}.gps-parts-button{border-color:#bfdbfe;color:#1d4ed8}.gps-parts-button:hover,.gps-parts-reset-button:hover{background:#f8fafc}.gps-parts-advanced{display:grid;grid-template-columns:repeat(6,minmax(140px,1fr));gap:12px;margin:-4px 0 20px;padding:16px;border:1px solid #e5e7eb;border-radius:16px;background:#f8fafc}.gps-parts-pagination{margin-top:18px;display:flex;flex-direction:column;gap:12px}.gps-parts-pagination-bar{display:flex;justify-content:flex-end}.gps-parts-list{display:flex;flex-direction:column;gap:12px;width:100%}.gps-admin-parts-grid{display:grid;grid-template-columns:minmax(150px,.6fr) minmax(300px,1.35fr) minmax(260px,1fr) minmax(150px,.65fr) minmax(190px,.75fr) minmax(170px,.65fr);gap:20px;width:100%;align-items:center}.gps-parts-list-header{padding:0 18px 4px;color:#64748b;font-size:12px;font-weight:800;text-transform:uppercase;letter-spacing:.04em}.gps-part-card{width:100%;min-height:140px;display:block;border:1px solid #e5e7eb;border-radius:18px;background:#fff;box-shadow:0 10px 24px rgba(15,23,42,.06);padding:18px}.gps-part-col{min-width:0}.gps-part-thumb-wrap{display:flex;gap:12px;align-items:flex-start}.gps-part-thumb{position:relative;flex:0 0 120px;width:120px;height:96px;border:1px solid #e5e7eb;border-radius:14px;background:#f8fafc;overflow:hidden}.gps-part-thumb img{width:100%;height:100%;object-fit:cover;display:block}.gps-part-thumb__placeholder{display:flex;align-items:center;justify-content:center;height:100%;text-align:center;color:#94a3b8;font-size:12px}.gps-part-thumb__badge{position:absolute;right:7px;bottom:7px;border-radius:999px;background:rgba(248,250,252,.9);border:1px solid #e2e8f0;color:#64748b;padding:2px 7px;font-size:11px;font-weight:800}.gps-part-id{font-weight:800;color:#334155}.gps-part-title{color:#1e293b;font-size:14px;font-weight:700;line-height:1.35;text-decoration:none}.gps-part-title:hover{color:#2563eb;text-decoration:underline}.gps-part-muted{color:#64748b;font-size:12px;margin-top:5px;overflow-wrap:anywhere}.gps-part-number-row{display:flex;align-items:center;gap:6px;margin-top:8px}.gps-part-number{font-size:14px;font-weight:800;color:#334155}.gps-part-copy{display:inline-flex;width:22px;height:22px;align-items:center;justify-content:center;border:0;border-radius:999px;background:#f1f5f9;color:#64748b;cursor:pointer}.gps-part-copy:hover{background:#e2e8f0;color:#1e293b}.gps-part-copy svg{width:14px;height:14px}.gps-part-badge{display:inline-flex;align-items:center;border-radius:999px;background:#e2e8f0;color:#334155;font-size:11px;font-weight:800;line-height:1;padding:6px 8px;margin-top:8px;margin-right:6px}.gps-part-badge--context{background:#dcfce7;color:#166534}.gps-part-review{margin-top:8px;padding-top:8px;border-top:1px dashed #e2e8f0}.gps-part-actions{display:flex;flex-wrap:wrap;gap:7px}.gps-part-action{border-radius:999px;border:1px solid #bfdbfe;color:#1d4ed8;font-size:12px;font-weight:800;padding:7px 10px;text-decoration:none;white-space:nowrap;background:#fff}.gps-part-action--success{border-color:#bbf7d0;color:#15803d}.gps-part-empty{border:1px dashed #cbd5e1;border-radius:18px;padding:32px;text-align:center;color:#64748b;background:#fff}.gps-admin-channels.part-channel-list{width:100%!important;min-width:0!important;max-width:none!important}.gps-admin-storage{min-width:0!important}@media(max-width:1300px){.gps-parts-toolbar,.gps-parts-advanced,.gps-admin-parts-grid{grid-template-columns:1fr 1fr}.gps-parts-list-header{display:none}}@media(max-width:700px){.gps-parts-toolbar,.gps-parts-advanced,.gps-admin-parts-grid{grid-template-columns:1fr}.gps-parts-pagination-bar{justify-content:stretch}}
    </style>
    ...
    <div class="gps-parts-list">
        <div class="gps-parts-list-header gps-admin-parts-grid"><div>Część</div><div>Numery</div><div>Kanały sprzedaży</div><div>Magazyn</div><div>Status</div><div>Akcje</div></div>
        @forelse ($parts as $part)
            ...
            <div class="gps-part-card"><div class="gps-admin-parts-grid">
                <div class="gps-part-col"><div class="gps-part-thumb-wrap"><div class="gps-part-thumb">@if ($imageUrl)<img src="{{ $imageUrl }}" alt="Zdjęcie części #{{ $part->id }}" loading="lazy">@if ($images->count() > 1)<span class="gps-part-thumb__badge">{{ $images->count() }}</span>@endif @else <span class="gps-part-thumb__placeholder">Brak<br>zdjęcia</span>@endif</div><div><div class="gps-part-id">#{{ $part->id }}</div><span class="gps-part-badge">{{ $statusLabel }}</span>@if ($listContextBadge)<span class="gps-part-badge gps-part-badge--context">{{ $listContextBadge }}</span>@endif</div></div></div>
                <div class="gps-part-col"><a class="gps-part-title" href="{{ PartResource::getUrl('edit', ['record' => $part]) }}">{{ $part->name ?: 'Bez nazwy' }}</a><div class="gps-part-number-row"><span class="gps-part-number">{{ filled($number) ? $number : '—' }}</span>@if (filled($number))<button type="button" class="gps-part-copy" title="Kopiuj numer części" onclick="event.preventDefault(); navigator.clipboard?.writeText(@js($number));"><svg viewBox="0 0 20 20" fill="currentColor"><path d="M7 3.5A2.5 2.5 0 0 1 9.5 1h6A2.5 2.5 0 0 1 18 3.5v6a2.5 2.5 0 0 1-2.5 2.5h-6A2.5 2.5 0 0 1 7 9.5v-6Z"/><path d="M4.5 6A2.5 2.5 0 0 0 2 8.5v8A2.5 2.5 0 0 0 4.5 19h8a2.5 2.5 0 0 0 2.5-2.5V14h-2v2.5a.5.5 0 0 1-.5.5h-8a.5.5 0 0 1-.5-.5v-8a.5.5 0 0 1 .5-.5H7V6H4.5Z"/></svg></button>@endif</div>@if ($part->sku)<div class="gps-part-muted">SKU: {{ $part->sku }}</div>@endif @if ($part->oem_number)<div class="gps-part-muted">OEM: {{ $part->oem_number }}</div>@endif @if ($part->manufacturer_code)<div class="gps-part-muted">Kod producenta: {{ $part->manufacturer_code }}</div>@endif @if ($part->category?->name)<div class="gps-part-muted">Kategoria: {{ $part->category->name }}</div>@endif @if ($part->car)<div class="gps-part-muted">Auto: {{ PartResource::carLabel($part->car) }}</div>@endif</div>
                <div class="gps-part-col">@include('filament.resources.parts.table-channels', ['part' => $part])</div>
                <div class="gps-part-col"><div class="gps-part-muted">Miejsce</div><div>{{ $part->storageLocation?->name ?: 'Brak lokalizacji' }}</div><div class="gps-part-muted">Ilość: {{ $part->quantity ?? 0 }}</div></div>
