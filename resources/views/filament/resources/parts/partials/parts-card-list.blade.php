...
<div class="gps-parts-list">
        ...
        @forelse ($parts as $part)
            ...
            <div class="gps-part-card"><div class="gps-admin-parts-grid">
                ...
                <div class="gps-part-col"><div><span class="gps-part-status-text {{ Part::statusTextClass($part->status) }}">{{ $statusLabel }}</span></div>@if ($part->review_reason || $part->review_detected_at || $part->review_source)<div class="gps-part-review"><div class="gps-part-muted">{{ $part->review_reason }}</div><div class="gps-part-muted">Wykryto: {{ $part->review_detected_at?->format('Y-m-d H:i') ?: '—' }}</div><div class="gps-part-muted">Źródło: {{ $part->review_source ?: '—' }}</div></div>@endif</div>
