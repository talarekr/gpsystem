...
<div class="gps-parts-list">
        ...
        @forelse ($parts as $part)
            ...
            <div class="gps-part-card"><div class="gps-admin-parts-grid">
                ...
                <div class="gps-part-col"><div class="gps-part-status-row"><span class="gps-part-status-text {{ Part::statusTextClass($part->status) }}">{{ $statusLabel }}</span><details class="gps-part-availability"><summary class="gps-part-availability__summary" title="Zmień lokalną dostępność">+</summary><select class="gps-part-availability__select" aria-label="Lokalna dostępność części #{{ $part->id }}" wire:change="updateLocalAvailability({{ $part->id }}, $event.target.value)"><option value="">—</option><option value="0" @selected($part->adminLocalAvailability() === 'sold')>0</option><option value="1" @selected($part->adminLocalAvailability() === 'for_sale')>1</option></select></details></div>@if ($part->review_reason || $part->review_detected_at || $part->review_source)<div class="gps-part-review"><div class="gps-part-muted">{{ $part->review_reason }}</div><div class="gps-part-muted">Wykryto: {{ $part->review_detected_at?->format('Y-m-d H:i') ?: '—' }}</div><div class="gps-part-muted">Źródło: {{ $part->review_source ?: '—' }}</div></div>@endif</div>
