...
@once
    <style>
        ...
        .gps-manual-marketplace-links__row{position:relative;display:flex;align-items:center;justify-content:space-between;gap:8px;white-space:nowrap}
        .gps-manual-marketplace-links__toggle{list-style:none;display:inline-flex;width:18px;height:18px;align-items:center;justify-content:center;border:1px solid #cbd5e1;border-radius:999px;background:#fff;color:#2563eb;font-size:13px;font-weight:700;line-height:1;cursor:pointer}
        .gps-manual-marketplace-links__toggle::-webkit-details-marker{display:none}
        .gps-manual-marketplace-links__toggle:hover{background:#eff6ff;border-color:#93c5fd}
...
<div class="gps-manual-marketplace-links" aria-label="Ręczne mapowanie linków marketplace">
    @foreach ($manualLinkRows as $marketplace => $label)
        <div class="gps-manual-marketplace-links__row">
            ...
            @if ($part)
                <details>
                    <summary class="gps-manual-marketplace-links__toggle" title="Dodaj link {{ $label }}" aria-label="Dodaj link {{ $label }}">+</summary>
