@php
    ...
    $src = $image?->path ? (str_starts_with($image->path, 'http') ? $image->path : asset($image->path)) : null;
@endphp
<article class="sf-product-card">
    <a class="sf-product-card__image" href="{{ $productUrl }}">@if($src)<img src="{{ $src }}" alt="{{ $image->alt_text ?: $part->name }}" loading="lazy">@else<span>GPSwiss<br>brak zdjęcia</span>@endif</a>
