...
<div class="sf-container sf-page gp-product-page" id="product-{{ $part->id }}">
    ...
    <div class="sf-product-detail">
        ...
        <section class="sf-info-card"><h1>{{ $part->name }}</h1><p><strong>Numer części:</strong> {{ $part->part_number ?: $part->sku ?: '—' }}</p><p><strong>Stan:</strong> Używany / sprawdzony</p>@if($part->car_id)<a class="sf-link-box" href="{{ route('storefront.catalog', ['vehicle_model'=>trim(($part->car?->make ?? '').' '.($part->car?->model ?? ''))]) }}">Pokaż więcej części z tego pojazdu</a>@endif<div class="sf-trust"><span>Czas dostawy</span><span>Metody płatności</span><span>Zwrot do 14 dni zgodnie z regulaminem</span></div></section><aside class="sf-purchase"><span>Cena produktu</span><strong>{{ number_format((float)$part->price,2,',',' ') }} {{ $part->currency ?: 'PLN' }}</strong><p>Cena brutto. Najniższa cena z 30 dni dostępna przy finalizacji zamówienia.</p>@if((int) $part->quantity > 0 && ! in_array($part->status, ['sold', 'archived'], true))<form method="post" action="{{ route('storefront.cart.add', $part) }}">@csrf<button class="sf-purchase__cart" type="submit">Dodaj do koszyka</button></form>@else<button disabled>Produkt niedostępny</button><small>Brak w magazynie.</small>@endif<a href="mailto:biuro@gpswiss.pl">Masz pytanie? Skontaktuj się</a><small>Pomagamy w doborze części po numerze VIN / OEM.</small></aside></div><div class="sf-tabs"><section class="sf-details-section"><p class="sf-details-description">{!! nl2br(e($description)) !!}</p><h2>Informacje szczegółowe</h2>@if($details)<div class="sf-details-table">@foreach($details as $detail)<div class="sf-details-row"><div class="sf-details-label">{{ $detail['label'] }}</div><div class="sf-details-value">{{ $detail['value'] }}</div></div>@endforeach</div>@else<p>Szczegółowe informacje zostaną uzupełnione.</p>@endif</section></div>@if($related->isNotEmpty())<section class="sf-section"><div class="sf-section__head"><h2>Więcej części z tego pojazdu</h2></div><div class="sf-grid sf-grid--4">@foreach($related as $part) @include('storefront.partials.product-card', ['part'=>$part]) @endforeach</div></section>@endif</div>
@endsection
